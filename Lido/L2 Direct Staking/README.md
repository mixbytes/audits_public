...
- **Report replay (same-chain and cross-chain) was examined and is contained.** The CRE forwarder does not mark a reverted transmission as used, so a failed report's signed blob stays permissionlessly re-submittable, and `CREReceiver.onReport` adds no timestamp/nonce idempotency guard of its own. We confirmed this is not exploitable here. Same-chain: a transmission is keyed `keccak256(receiver, workflowExecutionId, reportId)` and marked consumed once it succeeds, so only reports that already reverted can be re-submitted - and the sole allow-listed call is the nullary `triggerSync()`, whose amount is recomputed on-chain and which is delay-rate-limited, so a replay can at most produce the one legitimate, due sync that would have happened anyway; it cannot manufacture extra successful syncs or fragment batches. Cross-chain: the four lanes share a `workflowOwner` and Arbitrum/Optimism/Base even share a forwarder address, but a report is signed by a specific DON config and the forwarder re-validates every signature against that lane's signer set on each call (`InvalidConfig` / `InvalidSigner`), so a report signed for one lane cannot be routed on another - the cross-chain `chainSelector` guard the CRE docs recommend is therefore not required given the nullary design. The residual is purely forward-looking: if the allow-list is ever extended to a non-nullary or non-idempotent call, the missing report-level idempotency guard would become load-bearing and should be added at that time.

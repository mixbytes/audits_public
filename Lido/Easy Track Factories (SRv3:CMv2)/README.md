...
UpdateStakingModuleShareLimits.sol (CSM) | [0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1](https://etherscan.io/address/0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1) | Ethereum
...
Each deployment's constructor arguments were cross-checked against the published deployment manifests (`deployed-sm-mainnet.json` / `deployed-sr-mainnet.json`) and the on-chain immutables read back from the contracts; all match exactly. For eight of the nine factories the reference is the manifest in [lidofinance/easy-track#125](https://github.com/lidofinance/easy-track/pull/125); for the corrected CSM `UpdateStakingModuleShareLimits` instance the reference is the updated manifest in [lidofinance/easy-track#128](https://github.com/lidofinance/easy-track/pull/128) (the original #125 entry for that factory held the misconfigured caps described below). The immutable configuration is additionally internally consistent and resolves to the expected Lido contracts on the canonical StakingRouter [`0xFdDf38947aFB03C621C71b06C9C70bce73f12999`](https://etherscan.io/address/0xFdDf38947aFB03C621C71b06C9C70bce73f12999): for each `SettleGeneralDelayedPenalty` instance the `accounting` immutable equals `module.ACCOUNTING()`; the `CreateOrUpdateOperatorGroup` `module`, `metaRegistry` and `stakingRouter` form a consistent bidirectional linkage (`curatedModule.META_REGISTRY() == metaRegistry`, `metaRegistry.STAKING_ROUTER() == stakingRouter`) with `allowedExternalModuleId = 1`; the `UpdateStakingModuleShareLimits` instance targets `stakingModuleId = 3` with per-motion caps `maxStakeShareLimitIncrease/Decrease = 50` bp (0.5%) and `maxPriorityExitShareThresholdIncrease/Decrease = 60` bp (0.6%); and the `AllowConsolidationPair` `consolidationMigrator`, `stakingRouter`, `sourceModuleId = 1` and `targetModuleId = 4` match the migrator's own getters.

**Corrected `UpdateStakingModuleShareLimits` (CSM) redeployment.** The `UpdateStakingModuleShareLimits` (CSM) factory listed above, [`0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1`](https://etherscan.io/address/0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1), is a corrected redeployment. The originally deployed instance, [`0x0C6703F1d8D9DdfB6c6e5F57b4f7432a6500D6D8`](https://etherscan.io/address/0x0C6703F1d8D9DdfB6c6e5F57b4f7432a6500D6D8), was configured with per-motion caps that were 10x larger than intended: `maxStakeShareLimitIncrease/Decrease = 500` bp and `maxPriorityExitShareThresholdIncrease/Decrease = 600` bp - i.e. 5.0% and 6.0% rather than the intended 0.5% and 0.6%. The discrepancy traces to a basis-point misreading in the source parameter proposal (500 bp being 5% rather than the stated 0.5%). Because the error went unnoticed, the misconfigured factory was already registered in the Easy Track factory registry by the earlier on-chain Vote #203.

The remediation is a governance-level correction rather than a code change: the factory contract's logic is identical (the caps are constructor immutables, not encoded logic), so the fix is a redeployment with the correct arguments plus a registry swap. The corrected factory [`0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1`](https://etherscan.io/address/0xde3e46E3129fA4e4e3f66c9024B0A3Ad509b27a1) is proposed to be added to the Easy Track registry with unchanged permissions while the misconfigured [`0x0C6703F1d8D9DdfB6c6e5F57b4f7432a6500D6D8`](https://etherscan.io/address/0x0C6703F1d8D9DdfB6c6e5F57b4f7432a6500D6D8) is removed, both in the next on-chain vote. We verified on-chain that the two instances share identical runtime bytecode aside from the immutable parameter slots, that the corrected instance reads back `maxStakeShareLimitIncrease/Decrease = 50` bp and `maxPriorityExitShareThresholdIncrease/Decrease = 60` bp, and that all other configuration is unchanged from the original — same `trustedCaller` (the CSM committee Safe [`0xC52fC3081123073078698F1EAc2f1Dc7Bd71880f`](https://etherscan.io/address/0xC52fC3081123073078698F1EAc2f1Dc7Bd71880f)), same `stakingRouter` [`0xFdDf38947aFB03C621C71b06C9C70bce73f12999`](https://etherscan.io/address/0xFdDf38947aFB03C621C71b06C9C70bce73f12999), and same `stakingModuleId = 3`. The corrected deployment manifest is published in [lidofinance/easy-track#128](https://github.com/lidofinance/easy-track/pull/128).

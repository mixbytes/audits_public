...
**Verification of fix between commits `a4bbe78` and `bdd0483`:** We reviewed the fix applied between commits `a4bbe78934bdec534c4b78871f0bdce57467eab1` and `bdd048352fe6719b0e1f998bffdc05c6607688e0`. The root cause was that `SubmitExitRequestHashesUtils.validateExitRequests()` – the shared validation library used by both `CuratedSubmitExitRequestHashes` and `SDVTSubmitExitRequestHashes` — retrieved a signing key via `getSigningKey()` but did not check whether the key had been deposited (marked as `used`). 

This allowed a malicious active Curated node operator to submit an exit request, which could be crafted to force on-chain full-withdrawal/exit requests for other Curated operators' validators.

The fix adds a `require(used, "UNUSED_PUBKEY")` check on the `used` flag returned by `getSigningKey()`, which rejects any signing key that has not yet gone through the deposit process. An attacker-registered key will never pass this check because `addSigningKeys()` stores it with `used = false`, and only the protocol's deposit pipeline sets the flag to `true` after a real deposit.

We confirm the fix is correct and sufficient to close the reported authorization gap.
...
Type| Solidity
Platform| EVM
Timeline| 25.06.2025 - 16.03.2026
...
16.03.2026 | bdd048352fe6719b0e1f998bffdc05c6607688e0 | Commit with Updates
   
#### Mainnet Deployments

File| Address | 
--- | --- |
CuratedSubmitExitRequestHashes | [0x4F716AD3Cc7A3A5cdA2359e5B2c84335c171dCde](https://etherscan.io/address/0x4F716AD3Cc7A3A5cdA2359e5B2c84335c171dCde) 

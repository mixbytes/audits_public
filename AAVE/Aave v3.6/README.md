...
The upgrade payload contracts (`UpgradePayload.sol` and `UpgradePayloadMainnetCore.sol`) for upgrading the protocol to version 3.6 were reviewed based on commit [8402bf6](https://github.com/bgd-labs/protocol-v3.6-upgrade/commit/8402bf6626a9cfbad3698b1912b211e290a1b9f2). The upgrade logic correctly updates the Pool and PoolConfigurator implementations, upgrades the AToken and VariableDebtToken implementations for all reserves, sets the `ltvzeroBitmap` for assets with LTV=0 in eMode categories, and preserves backward compatibility. The Ethereum Mainnet Core pool includes additional handling for the GHO and AAVE tokens to maintain compatibility and delegation-related behavior. Overall, the upgrade payloads behave as expected.

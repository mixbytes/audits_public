...
- **StrategyTemplate Re-review and LayerZero `nextNonce` Update.** A follow-up review of the shared `StrategyTemplate` base (platform commit `6863b8a`) surfaced low-severity, defense-in-depth issues in the emergency-exit and swap-allowance paths, added as Low #44–#45. Neither relates to the changes introduced in the changes; both are pre-existing properties of the `StrategyTemplate` base surfaced during the full re-review. Separately, the LayerZero adapter's `nextNonce` change (lz-v2 commit `c28061e`) correctly hardens delivery ordering on-chain — it enforces gap-free per-channel nonces via `nextNonce` / `_acceptNonce`, complementing the `addExecutorOrderedExecutionOption` the adapter already used — but, matching the canonical LZ OApp example, it omits the `skipInboundNonce` recovery helper the LZ docs prescribe. Since the synchronous receive path (`MessageRouter` → `ContainerPrincipal` / `ContainerAgent.receiveMessage`) reverts when the target container is paused, in emergency/reshuffling, or off-status, a head-of-line revert stalls the whole shared channel; transient reverts self-heal on retry, and a would-be permanent revert is cleared by upgrading the (upgradeable) `MessageRouter` / Container so the message processes — a low-severity robustness gap. This is added as Low #46. The latest remediation round addressed all newly reported low-severity findings from the StrategyTemplate and LayerZero follow-up review. The reviewed remediation diff was also checked for regressions, and no new reportable vulnerabilities were identified.
...
Type| Solidity
Platform| EVM
Timeline| 12.01.2026 - 08.07.2026
...
26.06.2026 | 6863b8a48ceb0532dce67a0fbbd48b3ea7e08286 | Commit for strategy-template re-review (platform)
30.06.2026 | c28061e4f3767826337f9b72d03f0f5641a233b5 | Commit for re-audit (lz-v2)
07.07.2026 | 30f27218e890a5c69b53b93d64c5fce52fbe8294 | StrategyTemplate re-audit fixes (platform)
07.07.2026 | a6ed2b5c1418076325bd6193971f7f7eea16797c | LayerZero ordered-delivery recovery fix (lz-v2)
...
#### 44. `StrategyTemplate.emergencyExit()` — Failure Handling Is Unreachable Unless `emergencyExitMaxSlippage == MAX_BPS`

##### Status
Fixed in https://github.com/ShiftDeFi/shift-defi-platform/commit/30f27218e890a5c69b53b93d64c5fce52fbe8294

##### Description
[`StrategyTemplate.emergencyExit()`](https://github.com/ShiftDeFi/shift-defi-platform/blob/6863b8a48ceb0532dce67a0fbbd48b3ea7e08286/contracts/StrategyTemplate.sol#L468-L526) self-calls `tryEmergencyExit()` and catches an adapter revert into `isExitSuccess`, so the strategy can be parked in NAV-resolution mode (emitting `EmergencyExitFailed`) instead of reverting. However, the destination-NAV slippage gate runs unconditionally **before** that branch is evaluated:
...
if (vars.isExitSuccess) {
    if (share == MAX_BPS) { _acceptNav(toStateId); }
...
On a caught adapter failure no funds move, so `toStateNavAfterExit == toStateNavBeforeExit`, and the `require` passes only when `minNavDelta == 0`. The entry gate admits `minNavDelta == 0` only when `emergencyExitMaxSlippage == MAX_BPS`. Therefore, under any slippage-protected configuration (`emergencyExitMaxSlippage < MAX_BPS`), a genuine adapter `redeem` revert makes the **entire** `emergencyExit()` revert — rolling back `startEmergencyResolution()` and the resolution flag — instead of reaching the failure-handling branch the catch exists to serve. The operator cannot park the strategy / mark its NAV unresolved during the exact incident the mechanism is intended for; the only workaround is to widen the cap to `MAX_BPS` and retry, which simultaneously disables the destination-NAV floor.

##### Recommendation
We recommend evaluating `isExitSuccess` before enforcing the destination-NAV floor: apply the floor only on the success branch, and on the caught-failure branch skip it, emit `EmergencyExitFailed`, and remain in resolution mode — for any configured `emergencyExitMaxSlippage`.



---

#### 45. `StrategyTemplate` — Dangling Router Allowance After a No-Route Swap
...
[`_swapToInputTokens()`](https://github.com/ShiftDeFi/shift-defi-platform/blob/6863b8a48ceb0532dce67a0fbbd48b3ea7e08286/contracts/StrategyTemplate.sol#L621-L636) calls `forceApprove(swapRouter, amountIn)` and then `tryPredefinedSwap(...)` with `mustSucceed = false`. If no route exists the swap pulls nothing, leaving a non-zero `tokenIn → swapRouter` allowance dangling until the next successful swap of that token.

##### Recommendation
We recommend resetting the router allowance to zero (`forceApprove(swapRouter, 0)`) after the swap.
...
#### 46. `ShiftOApp` Ordered Delivery Omits the `skipInboundNonce` Recovery Helper

##### Status
Fixed in https://github.com/ShiftDeFi/shift-defi-message-adapter-lz-v2/commit/a6ed2b5c1418076325bd6193971f7f7eea16797c

##### Description
Commit `c28061e` opts `ShiftOApp` into LayerZero ordered delivery (`nextNonce` = `receivedNonce + 1`, enforced by `_acceptNonce`), matching the canonical LZ example but omitting its `skipInboundNonce` helper. Under ordered delivery a message that reverts at the head of the queue stalls the whole shared `(srcEid, sender)` channel (all Containers) until it is resolved, and the synchronous receive path (`_lzReceive` → `MessageRouter.receiveMessage` → `ContainerPrincipal` / `ContainerAgent.receiveMessage`) reverts when the target Container is paused, in emergency/reshuffling, or off-status.

These stalls are recoverable. The Endpoint keeps the payload verified, so a transient revert (pause / emergency / reshuffling) self-heals once the condition clears and the message is retried — no intervention. A would-be permanent revert (a malformed payload or an unrecoverable status) is cleared by upgrading the receiving `MessageRouter` / Container — both upgradeable — so the message processes and the nonce advances. The only gap is that `endpoint.skip` alone does not advance the app's `receivedNonce` (per the LZ "Keeping nonces in sync" docs), so the lighter `skipInboundNonce` recovery the LZ example ships is unavailable here.

##### Recommendation
We recommend adding `skipInboundNonce(srcEid, sender, nonce)` as in the LZ example — calling `endpoint.skip(...)` and advancing `receivedNonce` in the same call (per LayerZero, "Message Ordering — Keeping nonces in sync": https://docs.layerzero.network/v2/concepts/message-ordering#keeping-nonces-in-sync) — or making the receive path non-blocking, so a stalled message can be cleared without an upgrade.

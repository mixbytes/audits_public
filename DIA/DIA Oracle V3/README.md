...
**Unbounded operator-submitted values were addressed post-audit in `DIAOracleV3MetaFairValueField`.** During the audit, oracles and `ValueStore` operators were treated as trusted entities expected not to submit erroneous data, so on-chain sanity checks on submitted values were considered redundant. The client subsequently asked to remove the possibility of malformed data affecting aggregation regardless of that trust assumption. The root cause is the same in every case: `ValueStore` accepts arbitrary `uint256` values, while the metacontract performs unchecked arithmetic on them - `keyMain + 1` in both insertion sort comparisons, `ts + timeoutSeconds` in the freshness check outside the `try/catch` scope, `fairSum += fairValues[i]` when selecting the sort dimension, and `(a + b + 1) / 2` when averaging the two middle elements for an even number of live stores. Any of these reverts outside the `try/catch` scope, so a single extreme value can block `getMedianValues` for all keys. Related to this, the median could be computed from a single surviving non-zero entry once leading zeros were skipped, independently of the configured `threshold`.

**The fixes were implemented entirely in the metacontract and re-reviewed.** They were delivered in [PR #33](https://github.com/diadata-org/fair-value/pull/33), with the contract changes in commits [`c2782b0`](https://github.com/diadata-org/fair-value/commit/c2782b0555644c5a703f209907f58f4502d548ea) and [`f606814`](https://github.com/diadata-org/fair-value/commit/f606814dd7da85fbeecb02a341cdc2ad95bdfc51) and the accompanying test updates in [`91c11cd`](https://github.com/diadata-org/fair-value/commit/91c11cd) and [`69dd02c`](https://github.com/diadata-org/fair-value/commit/69dd02ca1ac9cc43c9d369a5940bada0ab73054b). Because `ValueStore` is already deployed across multiple nodes, the client chose not to upgrade it and instead applied all validation in `DIAOracleV3MetaFairValueField` - `ValueStore.sol` is unchanged. `_collectValues` now drops any entry whose `fairValue`, `usdValue`, numerator, or denominator exceeds `type(uint128).max`, which matches the `SafeCast.toUint128` already applied downstream, and the freshness check was rewritten as `block.timestamp - ts > timeoutSeconds` with an explicit rejection of future-dated timestamps. Both insertion sort comparisons use `main[prev] <= keyMain`, the sort-dimension selection was reworked to avoid the unbounded sum (and subsequently removed altogether, see below), and even-count averaging goes through a helper that never forms the intermediate sum. The median calculation now reverts with `ThresholdNotMet` when the number of entries actually contributing to the median falls below `threshold`. 

The client subsequently removed the implicit sort-dimension fallback entirely in `3f61db0` (merged as `73d735b`): `_sortValues` was deleted, `getMedianValues` now always sorts by `fairValue`, and a separate `getMedianValuesByUsd` was added as an explicit usd-sorted entry point. The sort dimension is now chosen by the caller, which is the only party that knows whether a fair value is expected for a given key, rather than inferred from the data. Where a mixed set of values previously switched the whole set to a usd-sorted median silently, `getMedianValues` now reverts with `AllPrincipalEntriesZero`.

We consider the overflow class closed - it is guarded both at the input filter and in the arithmetic itself - and we independently confirmed that the new averaging helper is exactly equivalent to `(a + b + 1) / 2` across the full `uint256` range while never forming the intermediate sum. The full test suite passes at the fix commit. Three points remain worth addressing. First, `threshold` is never initialized: the constructor sets only `maxValueStores`, so `threshold` defaults to `0` until an owner calls `setThreshold`, and while it is `0` both the pre-existing `_ensureThresholdMet` check and the newly added median guard are inert. `setThreshold` additionally permits a value of `1`, which allows the same outcome. We recommend initializing `threshold` in the constructor and enforcing a minimum of `2`. Second, the new freshness check rejects any entry with `ts > block.timestamp`, whereas `ValueStore._validateTimestamp` deliberately accepts timestamps up to `MAX_TIMESTAMP_GAP` (1 hour) into the future. Rows that `ValueStore` considers valid are therefore silently dropped by the metacontract, and under clock skew affecting all feeders simultaneously `getMedianValues` can revert outright. The updated tests encode the new rejection as expected behavior, so the suite does not surface this; the two contracts should be aligned on a single timestamp policy. Third, since out-of-range values are skipped rather than rejected at write time, a call may return a median computed from fewer stores than consumers expect; this should be noted in the documentation and paired with off-chain monitoring of the `ValueUpdated` event to catch bad data at submission time.

**Integrators must handle `getMedianValues` reverting when stores disagree on whether `fairValue` is defined.** Per the client, `fairValue == 0` is a legitimate state rather than bad data - the contract also serves assets for which a fair value is not well-defined (for example, assets backed by several reserve currencies), where only the denominator is non-zero. Stores can therefore legitimately disagree on a single key. Following the removal of the implicit fallback, the sort dimension is no longer inferred from the data: `getMedianValues` always sorts by `fairValue` and reverts `AllPrincipalEntriesZero` when every entry is zero, while `getMedianValuesByUsd` must be called explicitly for assets where only a usd-denominated median is meaningful.

In our opinion reverting is preferable to returning a median derived from one reported value, and that `threshold >= 2` is the right configuration - the residual concern is discoverability rather than the revert itself. Since `getMedianValues` and `getValue` are `view` functions that integrators will treat as always-available, the integration documentation should state that a revert is an expected outcome rather than an outage, that consumers must handle it instead of assuming a value is always returned, which keys are expected to carry a fair value at all. We further recommend off-chain alerting on mixed `fairValue` populations for a single key so the condition is noticed before consumers encounter it, and a distinct error selector to separate this case from ordinary insufficient-data reverts on-chain.

**`getValue` always aggregates by `fairValue`, and is unusable for multi-reserve assets.** `getValue` routes all four actions through `getMedianValues`, so for example `getValue("usdValue:BTC")` returns the `usdValue` from the fairValue-median row rather than a usd-sorted median. Because `getMedianValues` reverts with `AllPrincipalEntriesZero` when every entry has `fairValue == 0`, and per the README `fairValue` is non-zero only for single-reserve assets, `getValue` reverts for all four actions - fairValue, usdValue, numerator and denominator - on multi-reserve assets. `getMedianValuesByUsd` is therefore not reachable through `getValue` at all for that asset class. The client confirmed this is intended: an off-chain attestor service selects which function to call based on `assetType`, and no multi-reserve assets are currently deployed, so the restriction is not yet exercised. We note that returning a coherent row rather than five independently computed medians is consistent with the client's position on `numerator`/`denominator` medians elsewhere in this report. Since `getValue` is the ABI-facing entry point and the constraint is recorded only in NatSpec, the integration documentation should state which actions are valid for which keys, and that consumers needing a usd-sorted median must call `getMedianValuesByUsd` directly. This becomes load-bearing the moment the first multi-reserve asset is onboarded: any integrator or future component that reaches for `getValue` by default will revert.
...
Type| Solidity
Platform| EVM
Timeline| 04.03.2026 - 08.09.2026
...
26.08.2026 | 69dd02ca1ac9cc43c9d369a5940bada0ab73054b | Commit with Updates (fair-value)
01.09.2026 | 73d735b7ba6898b2068f56d32ed13b544d1f8847 | Commit with Updates (fair-value)
...
Following the audit, `DIAOracleV3MetaFairValueField` was redeployed at `0x369cb935eD8B445980FCb068860452D38f9A7781` to carry the post-audit fixes from commit [`73d735b7ba6898b2068f56d32ed13b544d1f8847`](https://github.com/diadata-org/fair-value/commit/73d735b7ba6898b2068f56d32ed13b544d1f8847), replacing the earlier deployment at `0x5aCEc1713BD9E00498215950667EEB902410c0CC`. The new contract is a direct non-proxy deployment and is therefore immutable. We verified it against the audited sources: the runtime bytecode matches a build of commit `73d735b7ba6898b2068f56d32ed13b544d1f8847` byte for byte, and of the seven verified source files published for the deployment, six are identical to the repository. The seventh, `DIAOracleV3MetaFairValueField.sol` itself, differs only in NatSpec comments, with no executable change. All post-audit fixes are present in the deployed code, `getMedianValuesByUsd` is deployed and callable, and the live configuration is `threshold` 2, `timeoutSeconds` 86400 and 11 registered ValueStores. Aggregation was additionally checked against live data: the median returned on chain matches an independently computed median of the values reported by the registered stores at the same block.
...
DIAOracleV3MetaFairValueField | [0x369cb935eD8B445980FCb068860452D38f9A7781](https://explorer.diadata.org/address/0x369cb935eD8B445980FCb068860452D38f9A7781) | DIA LaserNet
...
##### Status
Fixed in https://github.com/diadata-org/fair-value/commit/69dd02ca1ac9cc43c9d369a5940bada0ab73054b
